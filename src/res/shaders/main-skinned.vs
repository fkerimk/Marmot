...
in vec4 vertexTangent;      // xyz = tangent, w = bitangent sign (+1 / -1)
...
out vec4 fragColor;
out vec3 fragNormal;
out vec4 fragTangent; // world-space tangent and bitangent signs
...
void main() {
    ...
    // Apply a skin to the tangent as well (it should only affect rotation; w=0)
    vec4 skinnedTangent =
    ...
    fragPosition = vec3(matModel * vec4(skinnedPosition.xyz, 1.0));
    fragTexCoord = vertexTexCoord;
    fragColor    = vertexColor;
    fragNormal   = normalize(vec3(matNormal * skinnedNormal));

    // World-space tangent and bitangent signs
    vec3 T = normalize(vec3(matNormal * skinnedTangent));
    fragTangent = vec4(T, vertexTangent.w);
